  ...
  return (
    <div className="flex flex-col items-center justify-center select-none w-full">
      ...
      <div
        ...
        className={`relative w-28 h-64 bg-gradient-to-b from-stone-900 via-stone-800 to-stone-950 border-4 border-amber-600/70 rounded-2xl shadow-2xl p-3 flex flex-col items-center justify-between cursor-pointer group transition-all duration-200 ${
          disabled || isSpinning
      ...
      >
        ...
        <div className="px-2 py-1 bg-stone-950 border border-stone-700 rounded text-[9px] font-pixel text-amber-400 tracking-wider uppercase shadow-inner">
          ONE-ARMED
        </div>

        {/* Vertical Mechanical Track Groove */}
        <div className="relative w-8 h-44 bg-stone-950 border-2 border-stone-700 rounded-full shadow-inner flex flex-col items-center overflow-hidden">
          ...
          <div
            className={`absolute z-10 flex flex-col items-center transition-transform ${
              isPulled
                ? 'translate-y-20 duration-300 ease-in'
                : 'translate-y-1 duration-500 ease-out group-hover:translate-y-0'
            }`}
          >
            {/* Glossy Red & Gold Ball Knob */}
            <div
              className={`w-14 h-14 rounded-full bg-gradient-to-tr from-red-700 via-red-500 to-red-400 border-4 border-amber-400 shadow-2xl flex items-center justify-center transition-transform ${
                isPulled ? 'scale-95 brightness-90' : 'group-hover:scale-105 group-hover:brightness-110'
              }`}
            >
              {/* Glossy highlight reflection */}
              <div className="w-4 h-4 bg-white/60 rounded-full -translate-x-2 -translate-y-2 blur-[1px]" />
            </div>

            {/* Brushed Metallic Steel Shaft */}
            <div className="w-4 h-24 bg-gradient-to-r from-stone-500 via-stone-200 to-stone-600 border-x-2 border-stone-700 shadow-md" />

            {/* Bottom Brass Slider Pivot Joint */}
            <div className="w-7 h-5 bg-gradient-to-b from-amber-500 to-amber-700 border-2 border-amber-300 rounded shadow-md" />
          </div>
        </div>
      ...
      <button
        ...
        className={`mt-5 w-full max-w-[160px] py-3.5 px-4 font-pixel text-xs tracking-wider uppercase rounded-xl border-4 transition-all shadow-xl flex items-center justify-center gap-2 ${
          disabled
