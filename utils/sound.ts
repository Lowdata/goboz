// Custom MP3 sound effects with retro 8-bit fallback using Web Audio API
...
class SoundEffects {
  ...
  private playAudioFile(src: string, fallback: () => void) {
  ...
  public playCoin() {
    this.playAudioFile('/coinsound.mp3', () => {
  ...
  public playNoMatch() {
    this.playAudioFile('/losssound.mp3', () => {
      if (!this.enabled) return;
      try {
        this.initCtx();
        if (!this.ctx) return;
